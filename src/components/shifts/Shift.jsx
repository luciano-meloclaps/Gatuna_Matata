...
const Shift = ({ shifts, setShiftHandler, usersInfo, setUsersInfoHandler }) => {
          ...
          : shifts
              ...
              .map((shift, index) => (
                <tr key={shift.id}>
                  ...
                  <Button
                    variant="success"
                    onClick={() => handleShow(shift.id)}
                  >
                    Reservar turno
                  </Button>

                  <Modal
                    show={showModal.current === shift.id}
                    onHide={handleClose}
                  >
                    <Modal.Header closeButton>
                      <Modal.Title>
                        Reserva para <DateFixed date={shift.date} />
                      </Modal.Title>
                    </Modal.Header>
                    <Form className="m-4">
                      <Form.Group className="mb-3" controlId="formBasicCatName">
                        ...
                        <Form.Control
                          type="text"
                          placeholder=""
                          onChange={setCatHandlerOnChange}
                        />
                      </Form.Group>
                      ...
                      <Form.Group className="mb-3" controlId="formBasicAddres">
                        ...
                        <Form.Control
                          type="text"
                          placeholder=""
                          onChange={setAddresHandlerOnChange}
                        />
                      </Form.Group>
                      ...
                      >
                        ...
                        <Form.Control
                          ...
                          onChange={setDescriptionHandlerOnChange}
                        />
                        <Form.Text className="text-muted">
                          Comida preferida, juguete preferido, caricia
                          preferida, etc.
                        </Form.Text>
                      </Form.Group>
                    </Form>
    ...
    const usersMapped = usersInfo.map((user, index) => (
      <tr key={user.id}>
        <th scope="row">{index}</th>
        <td>{user.name}</td>
        <td>{user.userType}</td>
        ...
        <Button
          onClick={async () => {
            await fetch(
              `https://gatunamatataservice-3kcg.onrender.com/users/${user.id}`,
              {
                method: "DELETE",
                headers: {
                  "content-type": "application/json",
            ...
            await fetch("https://gatunamatataservice-3kcg.onrender.com/users", {
              headers: {
                accept: "application/json",
              },
            })
            ...
            <Nav variant="pills" className="flex-column">
              ...
              {userData.userType !== "sitter" &&
                userData.userType !== "client" && (
                  <Nav.Item>
                    <Nav.Link eventKey="tertiary">Gestion de usuarios</Nav.Link>
                  </Nav.Item>
                )}
            </Nav>
          </Col>
                ...
                <table className="table table-hover">
                  ...
                  <tbody>
                    {shiftsTaken.length === 0
                      ? "Todavia nadie reservo un turno, carga tu disponibilidad!!"
                      : shiftsTaken}
                  </tbody>
                </table>
              </Tab.Pane>
